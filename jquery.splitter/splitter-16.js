 ...
 $.fn.splitter = function(args){
	...
	return this.each(function() {
		...
		var opts = $.extend({
			...
			splitterClass:	"splitter ui-split-widget ui-split-widget-content",
			paneClass:		"splitter-pane",
			barClass:		"splitter-bar",
			barNormalClass: "ui-split-state-default",			// splitbar normal
			barHoverClass:  "ui-split-state-hover",			// splitbar mouse hover
			barActiveClass: "ui-split-state-highlight",		// splitbar being moved
			barLimitClass:  "ui-split-state-error",			// splitbar at limit
		...
		if ( opts.dock ) {
			splitter
				.bind("toggleDock"+opts.eventNamespace, function() {
					...
					splitter.trigger(bar.hasClass(opts.barDockedClass)?"undock":"dock");
				})
				.bind("dock"+opts.eventNamespace, function(){
					var pw = A[0][opts.pxSplit]; 
					//if ( !pw ) return;
                    if(bar.hasClass(opts.barDockedClass)) return;
					bar._pos = pw;
					var x={}; 
